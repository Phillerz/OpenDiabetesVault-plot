...
def plot(cfg, outPath, dataset, plottedHeader, beginDate, duration):
    ...
    plt.yticks(list(range(1, len(plottedHeader) + 1)), plottedHeader)
...
def main():
    versionString = "OpenDiabetesVault-Availability-Plot v2.0"
    outPath = os.getcwd()
    csvFileName = ""
    ...
    if options.config:
        ...
        if not os.path.isfile(configFileName):
            input = input("[Config] Config file \'" + configFileName + "\' does not exist. Do you want to create it? (y/n)")
            if input != "y":
                exit(0)
    ...
    for h in parsedCsvHeader:
        for f in cfg["params"].get("ignoreFields").split(","):
            if re.match(f, h):
                ignoredFields.append(h)

    plottedHeader = list(set(parsedCsvHeader) - set(ignoredFields))
